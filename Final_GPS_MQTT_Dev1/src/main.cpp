...
void publishSerialData(char *serialData){
  ...
  client.publish("htl/4bhel/TayariGPS", serialData);
}
...
void displayINFO()
{
  ...
  if (millis() > nextTimeToExecuteDisplay)
  {
    ...
    String localizacion = String("\"name\":\"Tayari\"")+String(", \"lat\":") + String(gps.location.lat(), 6) +String(", \"lon\":") + String(gps.location.lng(), 6);
    char dato[localizacion.length() + 1];
    localizacion.toCharArray(dato, localizacion.length());
    client.publish("htl/4bhel/TayariGPS", dato);
